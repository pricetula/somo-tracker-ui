...
export async function createAnswers(body: AddAnswerRequest[]): Promise<ActionResult<Answer[]>> {
  try {
    ...
    if (!res.ok) return { success: false, error: "Failed to create answers.", code: res.status };
    return { success: true, data: await res.json() };
  } catch {
    return { success: false, error: "Unable to reach the server.", code: 503 };
  }
...
export async function deleteAnswers(ids: string[]): Promise<ActionResult<void>> {
  try {
    const res = await apiClient("/answers", { method: "DELETE", body: JSON.stringify({ ids }) });
    if (!res.ok) return { success: false, error: "Failed to delete answers.", code: res.status };
    return { success: true };
  } catch {
    return { success: false, error: "Unable to reach the server.", code: 503 };
  }
