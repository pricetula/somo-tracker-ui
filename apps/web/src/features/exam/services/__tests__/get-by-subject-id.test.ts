...
describe("getExamsBySubjectID", () => {
    ...
    it("calls authenticatedGet with the correct URI", async () => {
        ...
        await getExamsBySubjectID("subject-1");

        expect(authenticatedGet).toHaveBeenCalledTimes(1);
        expect(authenticatedGet).toHaveBeenCalledWith({ uri: "/exams/subject/subject-1" });
    });
    ...
    it("returns parsed exams data from the response", async () => {
        ...
        const result = await getExamsBySubjectID("subject-1");
    ...
    it("throws if authenticatedGet rejects", async () => {
        ...
        await expect(getExamsBySubjectID("subject-1")).rejects.toThrow("Auth failed");
    });
    ...
    it("throws if response.json() rejects", async () => {
        ...
        await expect(getExamsBySubjectID("subject-1")).rejects.toThrow("Invalid JSON");
    });
