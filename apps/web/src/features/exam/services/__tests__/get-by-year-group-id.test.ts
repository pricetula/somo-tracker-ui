...
import { getExamsByYearGroupID } from "../get-by-year-group-id";
import { Exam } from "../../types";

// Mock authenticatedGet module
jest.mock("@/features/auth/utils/authenticated-get");

describe("getExamsByYearGroupID", () => {
    ...
    it("calls authenticatedGet with the correct URI", async () => {
        ...
        await getExamsByYearGroupID("year-group-1");

        expect(authenticatedGet).toHaveBeenCalledTimes(1);
        expect(authenticatedGet).toHaveBeenCalledWith({ uri: "/exams/year-group/year-group-1" });
    });
    ...
    it("returns parsed exams data from the response", async () => {
        ...
        const result = await getExamsByYearGroupID("year-group-1");
    ...
    it("throws if authenticatedGet rejects", async () => {
        ...
        await expect(getExamsByYearGroupID("year-group-1")).rejects.toThrow("Auth failed");
    });
    ...
    it("throws if response.json() rejects", async () => {
        ...
        await expect(getExamsByYearGroupID("year-group-1")).rejects.toThrow("Invalid JSON");
    });
