...
import { getQuestionByID } from "../get-by-id";
...
describe("getQuestionByID", () => {
    ...
    it("returns parsed questions data from the response", async () => {
        ...
        const result = await getQuestionByID("exam-1");
    ...
    it("throws if authenticatedGet rejects", async () => {
        ...
        await expect(getQuestionByID("exam-1")).rejects.toThrow("Auth failed");
    });
    ...
    it("throws if response.json() rejects", async () => {
        ...
        await expect(getQuestionByID("exam-1")).rejects.toThrow("Invalid JSON");
    });
