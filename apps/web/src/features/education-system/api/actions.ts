...
export async function getEducationSystem(id: string): Promise<ActionResult<EducationSystem>> {
    try {
        ...
        if (!res.ok)
            return { success: false, error: "Failed to fetch education system.", code: res.status };
        return { success: true, data: await res.json() };
    } catch {
        return { success: false, error: "Unable to reach the server.", code: 503 };
    }
...
export async function createEducationSystems(
    ...
    try {
        ...
        if (!res.ok)
            return {
                success: false,
                error: "Failed to create education systems.",
                code: res.status,
            };
        return { success: true, data: await res.json() };
    } catch {
...
export async function updateEducationSystem(
    ...
    try {
        ...
        if (!res.ok)
            return {
                success: false,
                error: "Failed to update education system.",
                code: res.status,
            };
        return { success: true, data: await res.json() };
    } catch {
...
export async function deleteEducationSystems(ids: string[]): Promise<ActionResult<void>> {
    try {
        ...
        if (!res.ok)
            return {
                success: false,
                error: "Failed to delete education systems.",
                code: res.status,
            };
        return { success: true };
    } catch {
