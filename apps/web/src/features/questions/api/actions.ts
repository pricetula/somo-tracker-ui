...
export async function createQuestions(
    ...
    try {
        ...
        if (!res.ok)
            return { success: false, error: "Failed to create questions.", code: res.status };
        return { success: true, data: await res.json() };
    } catch {
        return { success: false, error: "Unable to reach the server.", code: 503 };
    }
...
export async function updateQuestion(body: UpdateQuestionRequest): Promise<ActionResult<Question>> {
    try {
        ...
        if (!res.ok)
            return { success: false, error: "Failed to update question.", code: res.status };
        return { success: true, data: await res.json() };
    } catch {
        return { success: false, error: "Unable to reach the server.", code: 503 };
    }
...
export async function deleteQuestions(ids: string[]): Promise<ActionResult<void>> {
    try {
        ...
        if (!res.ok)
            return { success: false, error: "Failed to delete questions.", code: res.status };
        return { success: true };
    } catch {
        return { success: false, error: "Unable to reach the server.", code: 503 };
    }
