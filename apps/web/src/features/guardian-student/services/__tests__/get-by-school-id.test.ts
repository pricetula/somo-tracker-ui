...
describe("getGuardianStudentsBySchoolID", () => {
    ...
    it("calls authenticatedGet with the correct URI", async () => {
        ...
        await getGuardianStudentsBySchoolID("exam-1");

        expect(authenticatedGet).toHaveBeenCalledTimes(1);
        expect(authenticatedGet).toHaveBeenCalledWith({ uri: "/guardian-student/school/exam-1" });
    });
    ...
    it("returns parsed guardian-student data from the response", async () => {
        ...
        const result = await getGuardianStudentsBySchoolID("exam-1");
    ...
    it("throws if authenticatedGet rejects", async () => {
        ...
        await expect(getGuardianStudentsBySchoolID("exam-1")).rejects.toThrow("Auth failed");
    });
    ...
    it("throws if response.json() rejects", async () => {
        ...
        await expect(getGuardianStudentsBySchoolID("exam-1")).rejects.toThrow("Invalid JSON");
    });
